...
def _impl(ctx):
    ...
    tool_file = _tool_file(ctx.attr.tool_wire, "wire")
    args = [
        "wire",
        "--kind",
        ctx.attr.kind,
...
wire_runner = rule(
    ...
    executable = True,
    attrs = {
        "kind": attr.string(mandatory = True, values = ["gen", "diff", "check"]),
        "modules": attr.label_list(allow_files = ["go.mod"]),
        "target_pkgs": attr.string_list(default = ["./cmd/server"]),
        "gocache_dir": attr.string(default = "_output/wire-go-build-cache"),
        "tool_wire": attr.label(cfg = "exec", default = "@wire_tool_wire//:tool"),
