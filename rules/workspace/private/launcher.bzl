...
def _impl(ctx):
    ...
    if ctx.attr.kind in ["sync", "drift_check"]:
        args.extend(["--go-work", ctx.file.go_work.short_path if ctx.file.go_work else "go.work"])
    if ctx.attr.gazelle_target:
        args.extend(["--gazelle-target", str(ctx.attr.gazelle_target.label)])
    if not ctx.attr.run_bazel_mod_tidy:
        args.extend(["--run-bazel-mod-tidy", "false"])
    if ctx.attr.kind == "go_clean":
        args.extend(["--output-dir", ctx.attr.output_dir])
...
workspace_runner = rule(
    ...
    attrs = {
        ...
        "output_dir": attr.string(default = "_output"),
